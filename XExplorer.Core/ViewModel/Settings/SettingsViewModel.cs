...
public partial class SettingsViewModel : ViewModelBase
{
    ...
    /// <returns>表示异步操作的任务。</returns>
    [RelayCommand]
    public async Task UnzipDirAsync()
    {
        var st = Stopwatch.StartNew();
        Processing = true;
        ...
        try
        {
            var first_dir = "99_资源收藏\\";
            first_dir = this.AdjustPath(first_dir, OS.MacCatalyst);
            var result = await FolderPicker.PickAsync(default);
            if (result != null)
            {
                if (result.Folder == null)
                ...
                var valid = result.Folder.Path; 
                valid = valid.Replace(AppSettingsUtils.Default.Current.Volume, string.Empty);                
                var fullName = result.Folder.Path;

                foreach (var vol in AppSettingsUtils.Default.Current.Volumes)
                {
                    valid = valid.Replace(vol, string.Empty);
                    fullName = fullName.Replace(valid, string.Empty);
                }

                valid = valid.Replace(first_dir, string.Empty); 
                valid = this.AdjustPath(valid, OS.MacCatalyst);
                valid = valid.Replace(first_dir, string.Empty);
